...
def compare_models(prompt: str) -> dict:
    ...
    """
    def _compute_cost(token_usage: dict, pricing_key: str) -> float:
        pricing = PRICING_1M_TOKENS[pricing_key]
        return (
    ...
    return {
        "gpt4o": {
            ...
            "cost": _compute_cost(gpt4o_usage, "gpt-4o"),
        ...
        "gpt4o_mini": {
            ...
            "cost": _compute_cost(gpt4o_mini_usage, "gpt-4o-mini"),
        ...
        "gemini_flash": {
            ...
            "cost": _compute_cost(gemini_usage, "gemini-2.5-flash"),
...
def streaming_chatbot() -> None:
    ...
    print("Gemini 2.5 streaming chatbot ready. Type 'quit' or 'exit' to stop.")
    while True:
...
def batch_compare(prompts: list[str]) -> list[dict]:
    ...
    for prompt in prompts:
        try:
            ...
            # Some mocks may define compare_models with no arguments.
...
def format_comparison_table(results: list[dict]) -> str:
    ...
    for result in results:
        ...
        for key in ["gpt4o", "gpt4o_mini", "gemini_flash"]:
            ...
            tokens = f"{row.get('input_tokens', 0)}/{row.get('output_tokens', 0)}"
            cost = row.get("cost", 0.0)
            lines.append(
                f"| {prompt} | {model_names.get(key, key)} | {response} | {latency:.2f} | {tokens} | {cost:.8f} |"
            )
    return "\n".join(lines)
